...
const CustomerConnectionHandler = require('./customerConnectionHandler');
const OperatorConnectionHandler = require('./operatorConnectionHandler');
...
class MessageRouter {
  ...
  _sendConnectionStatusToOperator (customerId, customerName, disconnected) {
    ...
    this.operatorRoom.emit(status, {customerId: customerId, customerName: customerName});
  ...
  _routeCustomer (utterance, customer, customerId) {
    ...
    return this._sendUtteranceToOperator(utterance, customer)
      .then(() => {
        ...
        History.insertHistory(customer.id, customer.name, utterance, 'customer');
      ...
      .then(response => {
        ...
        if (customer.mode === CustomerStore.MODE_AGENT) {
          ...
          History.insertHistory(customer.id, customer.name, speech, 'agent');
  ...
  _relayOperatorMessage (message) {
    this.operatorRoom.emit(AppConstants.EVENT_OPERATOR_MESSAGE, 
      this._operatorMessageObject(message.customerObject.customerId, 
        message.utterance, false));
    // We're using Socket.io for our chat, which provides a synchronous API. However, in case
    // you want to swich it out for an async call, this method returns a promise.
    return Promise.resolve();
  }
