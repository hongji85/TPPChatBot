...
const ChatConnectionHandler = require('./chatConnectionHandler');
...
class CustomerConnectionHandler extends ChatConnectionHandler {
  constructor (socket, messageRouter, onDisconnect) {
    ...
    this.attachNewCustomerHandler();
  }

  init (customerId, customerName) {
    console.log('A customer joined: ', this.socket.id);
    this.router._sendConnectionStatusToOperator(customerId, customerName)
    // Determine if this is a new or known customer
      .then(() => this.router.customerStore.getOrCreateCustomer(customerId, customerName))
      .then(customer => {
  ...
  attachNewCustomerHandler() {
    this.socket.on(AppConstants.EVENT_CUSTOMER_NAME, (name) => {
      this.init(this.socket.id, name);
      this.attachHandlers();
    }); 
  }
